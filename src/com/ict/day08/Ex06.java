...
public class Ex06 {
	/*
      클래스
      1. 정의 : 객체(오브젝트, Object)를 만들기 위한 모든 정보를 가지고 있는 파일(설계도면, 거푸집등으로 표현)
      2. 클래스가 가지고 있는 것
        - 멤버필드 : (변수와 상수), 데이터, 속성
        - 멤버 메소드 : 기능, 동작
      ...
      3. 클래스 구조 : 헤더, 멤버필드, 멤버메소드, 생성자
        1) 헤더 : [접근제한자] [클래스종류] class 클래스이름{} 
          가) 접근제한자 : 클래스, 멤버필드, 멤버메소드, 생성자 모두 맨 앞에 사용 가능( 생략 가능 )
      ...
      5. 멤버메서드 : 동작, 기능
        ...
        1)메서드 구성
           [접근제한자] [메서드종류] 반환형 메서드이름([인자=매개변수]){실행내용}
          가) 클래스 설명에서의 접근제한자와 같다 
          나) 메서드종류 : 일반적인 메서드는 이 부분 생략한다.
                       특정메서드일때 해당 예약어 사용 (static 예약어)
          다) 반환형 : 해당 메서드가 실행이 끝나면 호출한 곳으로 되돌아 간다.
                     되돌아 갈때 정보를 가지고 갈 수 있는데 이때 이 정보의 자료형을 말한다.
                     정보를 가져가지 않을때는 void라는 예약어를 쓴다
                     
        //메인 메서드 : 해당 메서드를 실행한 후에 호출한 JVM에게는 되돌아 가는데 아무것도 가져가지 않는다
      ...
      8. static과 instance : 멤버필드, 멤버메서드에 공통으로 해당되는 사항
        ...
        2) static
           ...
           - 반드시 static이라는 예약어를 사용한다.  
           - 객체 생성과 상관없이 호출 가능
           - 호출방법 : 클래스이름.멤버필드, 클래스이름.멤버메서드
           -          System.out , Math.max
           
                               
       지역변수 또는 메서드 인자에 만들어진 변수는 해당 메서드가 실행될때 만들어진다
       객체 생성할때는 만들어지지 않는다                       
